...
# 2 초	128 MB	4325	1954	1583	47.015%
# 문제
# 프렉탈 평면은 다음과 같이 커진다. 시간 0에서 프렉탈은 흰색 정사각형 하나이다. 단위 시간(1)이 진행될 때마다 N×N개의 크기가 동일한 단위 정사각형으로 나누어진다. 
# 만약 나누어진 정사각형이 흰색이라면 가운데 K×K 정사각형이 검정색으로 채워진다. N과 K는 둘 다 홀수이거나, 둘 다 짝수이다.
...
min_vertex = (n-k) // 2
max_vertex = n-1 - ((n-k) // 2)

def find(x,y):    
    for _ in range(s):
        qx, rx, qy, ry = x//n, x%n, y//n, y%n
        if (min_vertex <= rx <= max_vertex) and (min_vertex <= ry <= max_vertex):
            return "1"
        x, y = qx, qy
    return "0"
